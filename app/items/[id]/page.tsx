    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-900 p-4 md:p-10 flex flex-col md:flex-row items-center gap-10">
            ...
            <div className="relative w-full md:w-1/2 h-80 md:h-[500px] rounded overflow-hidden shadow-lg hover:scale-105 transition-transform duration-300">
                <Image src={`${process.env.NEXT_PUBLIC_API_URL}${item.image}`} alt={item.name} fill className="object-cover" />
            </div>
            ...
            <div className="w-full md:w-1/2 space-y-4">
                ...
                <button onClick={() => toast.success("Added to cart!")} className="mt-6 px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded shadow-lg transition-all duration-300">
                    Add to Cart
                </button>
                <button onClick={() => router.back()} className="mt-2 px-6 py-2 border border-indigo-600 hover:bg-indigo-50 dark:hover:bg-slate-700 rounded transition">
                    Back to Items
                </button>
            </div>
        </div>
